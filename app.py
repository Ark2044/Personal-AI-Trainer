...
class ProcessFrame:
    def __init__(self, thresholds, flip_frame = False):
        ...
        # Import thresholds based on mode (beginner or pro)
        if thresholds == 'beginner':
            self.thresholds = get_thresholds_beginner()
        elif thresholds == 'pro':
            self.thresholds = get_thresholds_pro()
        else:
...
#     threshold_mode = st.selectbox("Choose threshold mode", ["beginner", "pro"])

#     # Update frame processor if mode changes
#     if st.session_state.frame_processor is None or st.session_state.frame_processor.thresholds != threshold_mode:
#         st.session_state.frame_processor = ProcessFrame(threshold_mode, flip_frame=True)
...
#             frame_width = int(st.session_state.cap.get(3))
#             frame_height = int(st.session_state.cap.get(4))
#             # Create a VideoWriter to save the processed video
#             fourcc = cv2.VideoWriter_fourcc(*'XVID')
#             st.session_state.output_video = cv2.VideoWriter('output_video.avi', fourcc, 30.0, (frame_width, frame_height))
#         else:
...
#     # Start/Stop button
#     button_label = "Stop Capture" if st.session_state.is_capturing else "Start Capture"
#     st.button(button_label, on_click=toggle_capture)

#     # Main capture and processing loop
#     if st.session_state.is_capturing:
#         status_text.text("Capturing... Click 'Stop Capture' to end.")
...
#             if ret:
#                 # Process the frame
#                 processed_frame, play_sound = st.session_state.frame_processor.process(frame, pose_model)
...
#                 # Update progress bar (optional, as it might not be meaningful for live capture)
#                 progress_value = int(st.session_state.cap.get(cv2.CAP_PROP_POS_FRAMES) % 100)
...
#         if st.session_state.frame_processor:
#             st.write(f"Correct squats: {st.session_state.frame_processor.state_tracker['SQUAT_COUNT']}")
#             st.write(f"Incorrect squats: {st.session_state.frame_processor.state_tracker['IMPROPER_SQUAT']}")
...
def main():
    ...
    status_text = st.empty()
    progress_bar = st.empty()
    correct_count = st.empty()
    incorrect_count = st.empty()
    ...
    if st.session_state.is_capturing:
        ...
        while st.session_state.is_capturing:
            ...
            if ret:
                ...
                # Update squat counts
                correct_count.text(f"Correct squats: {st.session_state.frame_processor.state_tracker['SQUAT_COUNT']}")
                incorrect_count.text(f"Incorrect squats: {st.session_state.frame_processor.state_tracker['IMPROPER_SQUAT']}")
            else:
                st.error("Failed to capture video")
                st.session_state.is_capturing = False
                break
