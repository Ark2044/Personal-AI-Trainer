...
# Create an instance of the ProcessFrame class with the chosen threshold mode
while True:
    threshold_mode = input("Choose the threshold mode (beginner or pro): ")
    if threshold_mode in ['beginner', 'pro']:
        break
    else:
        print("Invalid input. Please enter 'beginner' or 'pro'.")

frame_processor = ProcessFrame(threshold_mode, flip_frame=True)
...
# Create a VideoCapture object to read a recorded video file
# video_file = 'girl.mp4'
...
# Get the frame width, frame height, and frame rate of the input video
# Get the frame dimensions of the input video
frame_width = int(cap.get(3))
frame_height = int(cap.get(4))

# Create a VideoWriter to save the processed video with the same dimensions
fourcc = cv2.VideoWriter_fourcc(*'XVID')
output_video = cv2.VideoWriter('output_video.avi', fourcc, 30.0, (frame_width, frame_height))
while cap.isOpened():
    ret, frame = cap.read()
    if not ret:
        break
    ...
    # Process the frame
    processed_frame, play_sound = frame_processor.process(frame, pose_model)

    if frame_width > 0 and frame_height > 0:
        # Resize the processed frame to match the dimensions of the output video
        processed_frame = cv2.resize(processed_frame, (frame_width, frame_height))
    else:
        print("Invalid frame dimensions: width={}, height={}".format(frame_width, frame_height))

    # Display the processed frame
    # Create an OpenCV named window with a specific size
    cv2.namedWindow('Processed Frame', cv2.WINDOW_NORMAL)  # Use WINDOW_NORMAL to allow window resizing
    # cv2.resizeWindow('Processed Frame', 500, 500)  # Set the desired width and height

    # Then, inside your while loop, you can display the processed frame as before
    cv2.imshow('Processed Frame', processed_frame)
    output_video.write(processed_frame)  # Write the frame to the output video
